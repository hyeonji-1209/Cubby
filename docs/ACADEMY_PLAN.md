...
### 0.3 설정에 따른 UI 변화
| 설정 | 개인 수업 (OFF) | 반 수업 (ON) |
|------|-----------------|--------------|
| 수업 방식 | 회원 탭에서 개인별 관리 | "반 관리" 탭 표시 |
| 연습실 | 연습실 탭 숨김 | "연습실" 탭 표시 |
| 보호자 | 수강생만 가입 | 보호자/수강생 선택 가입 |

> 💡 **개인 수업 모드**: 학생 한 명씩 일정/진도를 관리 (1:1 레슨 학원에 적합)
> 💡 **반 수업 모드**: 반(클래스) 단위로 학생을 묶어 관리 (그룹 수업 학원에 적합)
...
```
방법 1: 회원별 담당강사 지정
  GroupMember 테이블에 instructorId 추가
  → 관리자가 각 회원에게 강사 배정

방법 2: 반 리더 = 담당강사
  반(SubGroup)의 리더가 해당 반 수강생의 담당강사
  → 반 운영 시 자동 배정

→ 둘 다 지원: 반이 있으면 반 리더, 없으면 개별 지정
```
