...
import type { GroupMember, Attendance, LessonSchedule } from '@/types';
...
export const LessonPanel = ({
  ...
  }, [isOpen, member, loadTodayLesson, loadHistory]);
  ...
  return (
    ...
    >
      <div className="lesson-panel">
        {loading ? (
          <div className="lesson-panel__loading">로딩 중...</div>
        ) : error ? (
          <div className="lesson-panel__error">{error}</div>
        ) : (
          <>
            {/* 탭 */}
            <div className="lesson-panel__tabs">
              <button
                className={`lesson-panel__tab ${activeTab === 'today' ? 'active' : ''}`}
                onClick={() => setActiveTab('today')}
              >
                오늘 수업
              </button>
              <button
                className={`lesson-panel__tab ${activeTab === 'history' ? 'active' : ''}`}
            ...
            {activeTab === 'today' && todayRecord && (
              <div className="lesson-panel__today">
                {/* 수업 정보 */}
                <div className="lesson-panel__schedule-info">
                  <span className="lesson-panel__day">
                    {todaySchedule && DAYS_OF_WEEK[todaySchedule.dayOfWeek]}요일
                  </span>
                  <span className="lesson-panel__time">
                    {todayRecord.lessonStartTime} - {todayRecord.lessonEndTime}
                  </span>
                  {isLessonTime && (
                    <span className="lesson-panel__live">수업 중</span>
                  )}
                </div>

                {/* 출석 상태 */}
                <div className="lesson-panel__attendance">
                  {todayAttendance ? (
                    <div className={`lesson-panel__status lesson-panel__status--${todayAttendance.status}`}>
                      ...
                      {todayAttendance.checkedAt && (
                        <span className="lesson-panel__check-time">
                    ...
                    <div className="lesson-panel__status lesson-panel__status--pending">
                      출석 대기
                    </div>
                  )}
                  ...
                  {todayAttendance &&
                    ...
                    todayAttendance.status !== 'absent' && (
                      <button
                        className="lesson-panel__early-leave-btn"
                ...
                {isLessonTime && !todayAttendance && (
                  <div className="lesson-panel__qr">
                    {qrLoading ? (
                      <div className="lesson-panel__qr-loading">QR 생성 중...</div>
                    ) : qrToken ? (
                      <>
                        <QRCodeSVG
                          ...
                          marginSize={2}
                        />
                        <p className="lesson-panel__qr-hint">
                          학생이 스캔하여 출석 체크
                        </p>
                      </>
                    ) : (
                      <button
                        className="lesson-panel__qr-generate"
                        onClick={generateQR}
                      >
                        QR 생성
                      </button>
                ...
                {todayRecord.previousContent && (
                  <div className="lesson-panel__section">
                    <label className="lesson-panel__label">지난 수업 내용</label>
                    <div className="lesson-panel__readonly">
                      {todayRecord.previousContent}
                    </div>
                  </div>
                )}
                ...
                {/* 이번 수업 내용 */}
                <div className="lesson-panel__section">
                  <label className="lesson-panel__label">이번 수업 내용</label>
                  <textarea
                    className="lesson-panel__textarea"
                ...
                {/* 과제 */}
                <div className="lesson-panel__section">
                  <label className="lesson-panel__label">과제</label>
                  <textarea
                    className="lesson-panel__textarea"
                ...
                {/* 비고 */}
                <div className="lesson-panel__section">
                  <label className="lesson-panel__label">비고</label>
                  <textarea
                    className="lesson-panel__textarea"
                ...
                {/* 저장 버튼 */}
                <button
                  className="lesson-panel__save-btn"
                  onClick={handleSave}
                  disabled={saving}
                >
                  {saving ? '저장 중...' : '저장'}
            ...
            {activeTab === 'history' && (
              <div className="lesson-panel__history">
                {historyLoading ? (
                  <div className="lesson-panel__loading">로딩 중...</div>
                ) : previousRecords.length === 0 ? (
                  <div className="lesson-panel__empty">수업 기록이 없습니다.</div>
                ) : (
                  <div className="lesson-panel__history-list">
                    {previousRecords.map((record) => (
                      <div key={record.id} className="lesson-panel__history-item">
                        <div className="lesson-panel__history-header">
                          <span className="lesson-panel__history-date">
                            {record.lessonDate}
                          </span>
                          <span className="lesson-panel__history-time">
                            {record.lessonStartTime} - {record.lessonEndTime}
                          </span>
                        </div>
                        {record.currentContent && (
                          <div className="lesson-panel__history-content">
                            <strong>수업 내용:</strong> {record.currentContent}
                          </div>
                        )}
                        {record.homework && (
                          <div className="lesson-panel__history-homework">
                            <strong>과제:</strong> {record.homework}
                          </div>
                        )}
                        {record.note && (
                          <div className="lesson-panel__history-note">
